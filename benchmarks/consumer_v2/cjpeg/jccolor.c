...
typedef struct {
  ...
  e_s32 * rgb_ycc_tab;        /* => table for RGB to YCbCr conversion */
} my_color_converter;
...
#define CBCR_OFFSET    ((e_s32) CENTERJSAMPLE << SCALEBITS)
#define ONE_HALF    ((e_s32) 1 << (SCALEBITS-1))
#define FIX(x)        ((e_s32) ((x) * (1L<<SCALEBITS) + 0.5))
...
rgb_ycc_start (j_compress_ptr cinfo)
{
  ...
  cconvert->rgb_ycc_tab = rgb_ycc_tab = (e_s32 *)
    (*cinfo->mem->alloc_small) ((j_common_ptr) cinfo, JPOOL_IMAGE,
                (TABLE_SIZE * SIZEOF(e_s32)));
         ...
         JDIMENSION output_row, int num_rows)
{
  my_cconvert_ptr cconvert = (my_cconvert_ptr) cinfo->cconvert;
  register int r, g, b;
  register e_s32 * ctab = cconvert->rgb_ycc_tab;
